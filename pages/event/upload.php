	...
	if(!empty($event))
	{
		...
		if(count($files) > 0)
		{
			...
			foreach($files as $file)
			{
				...
				{
					$currentFiles .= '	<li><a href="'.EVENT_MANAGER_BASEURL.'/event/file/'.$event->getGUID().'/'.$file->time_uploaded.'/'.$file->file.'">'.$file->title.'</a>
											<a onclick="javascript: if(!confirm(\''.elgg_echo('question:areyousure').'\')){return false;}" href="'.elgg_add_action_tokens_to_url($CONFIG->wwwroot.'action/event_manager/event/deletefile?guid='.$event->getGUID().'&file='.$file->file.'&t='.$file->time_uploaded).'">
